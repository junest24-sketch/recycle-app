...
function ShareLineBtn({ elementId, title = "สรุปข้อมูล" }) {
  ...
  return (
    <button onClick={share} title="แชร์เข้า LINE" style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 10px", borderRadius: 6, border: "none", background: "#06C755", cursor: "pointer", fontSize: 12, color: "#fff", fontWeight: 600 }}>
      <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.02 2 11c0 3.07 1.63 5.79 4.14 7.5L5 22l4.22-2.23C10.08 20.24 11.03 20.4 12 20.4c5.52 0 10-4.02 10-8.9C22 6.02 17.52 2 12 2z"/></svg>
      LINE
    </button>
  );
}
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #6B1A1A 0%, #A52828 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Noto Sans Thai', 'Inter', system-ui, sans-serif" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 20, padding: "40px 36px", width: "100%", maxWidth: 400, boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
          ...
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 22, color: "#6B1A1A" }}>{shopProfile.name || "changlo"}</div>
...
const { paged, page, setPage, totalPages, total, start, end } = usePagination(filtered);
  ...
  const save = () => {
    ...
    if (modal.mode === "edit") {
      ...
      const origRemaining = grandTotal(modal.item) - paidTotal(modal.item);
      const isFullyPaid = hasPaid && origRemaining <= 0.009; // เผื่อ error ทศนิยมเล็กน้อย
      if (isFullyPaid) {
        alert(`⚠️ ไม่สามารถแก้ไขได้!\n\nใบรับสินค้า "${modal.item.id}" ชำระครบแล้ว\nกรุณายกเลิกการชำระก่อน จึงจะแก้ไขได้`);
        return;
      }
...
function WithdrawalsTab({ products, purchases, sales, setSales, withdrawals, setWithdrawals, inventory, customers, companySettings }) {
  ...
  const splitToVAT = (lot, itemIdx) => {
    ...
    const updatedWithdrawals = updatedItems.length === 0
      ? withdrawals.filter((w) => w.id !== lot.id)
      : withdrawals.map((w) => (w.id === lot.id ? { ...w, items: updatedItems } : w));

    setSales(syncWithdrawalsToSales(sales, updatedWithdrawals));
    setWithdrawals(updatedWithdrawals);

    // เปิดฟอร์ม "สร้างใบเบิกสินค้าใหม่" พรีฟิลจำนวนที่แยกออกมาให้ทันที ใช้วันที่เดิมของใบเบิกต้นฉบับ
    // (ไม่ใช้วันนี้ เพื่อไม่ให้ต้นทุนไปตกในเดือนที่ผิดในหน้างบเดือน)
    setForm({
      ...blankForm(),
      date: lot.date,
...
function ExpensesTab({ expenses, setExpenses, storeBankAccounts, loans, setLoans, expenseCategories, setExpenseCategories, companySettings, customers }) {
  ...
  const save = () => {
    ...
    // ถ้าแก้ไข และชำระครบแล้ว ห้ามแก้ (ถ้ายังชำระไม่ครบ ยังแก้ไขได้ปกติ)
    if (modal.mode === "edit") {
      const hasPaid = (modal.item.payments || []).length > 0;
      const origTotals = calcTotals(modal.item);
      const origPaid = (modal.item.payments || []).reduce((s, p) => s + (Number(p.amount) || 0), 0);
      const isFullyPaid = hasPaid && (origTotals.net - origPaid) <= 0.009; // เผื่อ error ทศนิยมเล็กน้อย
      if (isFullyPaid) {
        alert(`⚠️ ไม่สามารถแก้ไขได้!\n\nค่าใช้จ่าย "${modal.item.id}" ชำระครบแล้ว\nกรุณายกเลิกการชำระก่อน จึงจะแก้ไขได้`);
        return;
      }
    }
    const cleaned = {
